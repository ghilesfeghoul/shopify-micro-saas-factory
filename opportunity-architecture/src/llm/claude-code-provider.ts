...
export class ClaudeCodeProvider implements LLMProvider {
  ...
  constructor(opts: { claudeBinary?: string; timeoutMs?: number; model?: string; useBare?: boolean } = {}) {
    ...
    this.timeoutMs = opts.timeoutMs ?? 30 * 60 * 1000; // 30 minutes — specs are larger than scans
    this.model = opts.model;
    this.useBare = opts.useBare ?? false; // Default false: use Pro/Max OAuth session
